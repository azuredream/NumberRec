...
def hello_world():
	...
	result = recnub(tva)
	#存入Cassandra
	imgname  = "测试图片"+str(idcount)
	img
	session.execute("""INSERT INTO users (id,imgname, img, time, result) VALUES (idcount, imgname, isrctrans,str(datetime.datetime.now()), str(result)); """)
		#每次存好显示数据表
	session.execute("""SELECT * FROM data;""")
	idcount = idcount+1;
